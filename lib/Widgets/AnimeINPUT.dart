...
class AnimeINPUT extends StatefulWidget {
  static const routeName = 'AnimeINPUT';
  const AnimeINPUT({super.key});

  @override
  State<AnimeINPUT> createState() => _AnimeINPUTState();
}

class _AnimeINPUTState extends State<AnimeINPUT> {
  ...
  void didChangeDependencies() {
    if (_istrue) {
      ...
      if (movieId != null) {
        _editmovie = Provider.of<AnimeFunctionClass>(context, listen: false)
            .getByid(movieId);
  ...
  Future<void> addMovies() async {
    ...
    try {
      if (movieId == null) {
        await Provider.of<AnimeFunctionClass>(context, listen: false)
            .addnewmovie(_newMovie)
            .then(
              (value) => Provider.of<AnimeFunctionClass>(context, listen: false)
                  .search(''),
            )
            .then((value) async {
          await Provider.of<DeviceTokens>(context, listen: false)
              .sendNotification('New Anime added', _newMovie.movieName);
        });
      } else {
        await Provider.of<AnimeFunctionClass>(context, listen: false)
